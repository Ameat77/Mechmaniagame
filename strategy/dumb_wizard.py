...
class DumbWizardStrategy(Strategy):
    ...
    def possible_moves(x, y, speed):
        ...
        for i in range(len(pairs)):
            ...
            if (inBounds(new_pos[0], new_pos[1])):
                moves.add(new_pos)
            new_pos = (x - pairs[i][0], y - pairs[i][1])
            if (inBounds(new_pos[0], new_pos[1])):
                moves.add(new_pos)
    ...
    def move_action_decision(self, game_state: GameState, my_player_index: int) -> Position:
        ...
        moves = DumbWizardStrategy.possible_moves(position[0], position[1], speed)
        best_move = DumbWizardStrategy.most_central(moves)
